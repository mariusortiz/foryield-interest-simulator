...
st.title("EUR Strategy + ForYield Interest")
...
with st.sidebar:
    ...
    start_price = st.number_input("Prix EUR de départ", min_value=0.01, value=1.0, step=0.01, format="%.2f")
    annual_app_pct = st.number_input("Appréciation EUR annuelle (%)", min_value=-99.0, max_value=500.0, value=0.0, step=0.1, format="%.2f")
    ...
    st.markdown("---")
    st.info("Par défaut, EUR = actif stable (prix 1, appréciation 0%).")
